    ...
    public partial class Form1 : Form
    {
        ...
        public void CrearProceso(int cantidadProcesos)
        {
            ...
            for (int i = 0; i < System.Convert.ToInt32(cantidadProcesos); i++)
            {
                ...
                if (ProcesosMemoria.Count + ProcesosBloqueados.Count < 5)
                {
        ...
        public void ListarTerminadosBCP(ListView listView, ArrayList array)
        {
            ...
            foreach (Proceso pros in array)
            {
                ...
                pros.tiempoFinalizacion = System.Convert.ToInt32(tiempoTranscurrido.Text);
                pros.tiempoRetorno = pros.tiempoFinalizacion - pros.tiempoLlegada;
                pros.tiempoEspera = pros.tiempoRetorno - pros.tiempoServicio;
        ...
        {
            quantumInt++;
            quantumtimeDisplay.Text = quantumInt.ToString();
            nuevos_tb.Text = ProcesosNuevos.Count.ToString();
            contadorGeneral++;
            contProceso++;
            ...
            if(contProceso == System.Convert.ToInt32(quantumValue.Text) && ProcesoActual.tiempoRestante > 0)
            {

                quantumInt = 0;
                ProcesoActual.tiempoRestante = System.Convert.ToInt32(tbTiempo.Text);
                ProcesosMemoria.Remove(ProcesoActual);
                tbID.Text = "";
                tbOp.Text = "";
                tbTiempo.Text = "";
                ProcesosMemoria.Add(ProcesoActual);
                contProceso = 0;
                ListarMemoria(tablaMemoria, ProcesosMemoria);
            }
        ...
        {
            CrearProceso(System.Convert.ToInt32( value_cantidad.Text));
            ListarMemoria(tablaMemoria, ProcesosMemoria);
        }
